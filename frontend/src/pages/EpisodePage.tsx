...
import type { EpisodeDetail, CharacterDetail } from "@/lib/types";
...
export default function EpisodePage() {
  ...
  const [isLoadingChars, setLoadingChars] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setChars([]);
        setShowAll(false);
        const item = await getById("episodes", Number(id));
        setData(item);
      } catch (e: any) {
  ...
  const allCharIds = useMemo(() => (data ? idsFromUrls(data.characters) : []), [data]);
  ...
  if (error) return <div className="p-6 text-red-600">Episode not found</div>;
  ...
  return (
    <PageShell>
      ...
      <div>
        ...
        <div className="text-gray-700">{data.episode} · {data.air_date}</div>
      </div>

      <section className="space-y-2">
        <h2 className="font-semibold">Characters ({allCharIds.length})</h2>
        {visibleCharIds.length === 0 ? (
          <div className="text-gray-500">No characters</div>
        ) : (
          <>
            ...
            {isLoadingChars && <div className="text-xs text-gray-500">Loading characters…</div>}
          </>
        )}
      </section>
    </PageShell>
